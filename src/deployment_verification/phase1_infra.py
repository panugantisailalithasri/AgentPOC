...
def run_cfn_stack_checks(service: ExpectedService, aws: AwsFacade) -> list[CheckResult]:
    """Check that a CFN stack exists and its status is healthy (*_COMPLETE, non-rollback).
    The actual AWS resources inside the stack are verified separately via the
    infra_export.variables entries (ECS cluster, ECR repos, ALB, S3 bucket, etc.)."""
